...
class PostController {
  ...
  public static createPost: RequestHandler = async (req, res) => {
    ...
    await UserModel.findOne({ id: post.author.id })
      .then(async (user) => {
        if (user == null) {
  ...
  public static addComment: RequestHandler = async (req, res) => {
    ...
    await UserModel.findOne({ id: comment.author.id })
      .then(async (user) => {
        if (user == null) {
          return res.status(400).json({ error: "Invalid user" });
        } else {
          await PostModel.findOne({ id: comment.post_id })
            .then(async (post) => {
              if (post == null) {
    ...
    await UserModel.findOne({ id: user.id })
      .then(async (user) => {
        if (!user) {
          ...
          await PostModel.findOne({ id: post_id })
            .then(async (post) => {
              if (post == null) {
                ...
                if (post.likes == null) {
                  ...
                  if (post.likes.includes(user.username)) {
                    const index = post.likes.findIndex((element) => {
              ...
              .then(async (post) => {
                ...
                await post
                  ...
                  .then(async () => {
                    return res
                      .status(200)
                      .json({ message: "Success removing post" });
                  })
                  .catch((e: Error) => {
                    return res.status(500).json({ error: e.name });
                  });
  ...
  public static editPost: RequestHandler = async (req, res) => {
    ...
    if (!post.content)
      return res.status(400).json({ error: "Post content is missing" });
    if (!post.created)
      return res.status(400).json({ error: "Created is missing" });
