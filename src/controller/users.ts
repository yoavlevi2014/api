...
class UserController {
  ...
  public static sendFriendRequest: RequestHandler = async (req, res) => {
    ...
    if (!to) return res.status(400).json({ error: "To user is missing" });

    if (!from) return res.status(400).json({ error: "From user is missing" });
            ...
            .then(async (fromUser) => {
              if (fromUser == null) {
                ...
                if (toUser.friends?.includes(fromUser.username)) {
                  return res
                    .status(400)
                    .json({ error: "Users are already friends" });
                } else {
                  ...
                  await FriendRequestModel.findOne({
                    ...
                    .then(async (request) => {
                      if (request == null) {
                        ...
                        return res
                          .status(403)
                          .json({ error: "Request already exists" });
                      }
                    })
                    .catch((error: Error) => {
                      throw error;
   ...
   *          description: Internal server error
   */
  public static getAllUsersFriendRequests: RequestHandler = async (
    req,
    res
  ) => {
   ...
   *          description: Internal server error
   */
  public static getAllUsersToFriendRequests: RequestHandler = async (
    req,
    res
  ) => {
    const username: string = req.params.user;
    ...
    if (username == null) {
      ...
      await UserModel.findOne({ username: username }).then(async (user) => {
        if (user == null) {
          ...
          await FriendRequestModel.find({ to_user: username })
            .then(async (requests) => {
              return res.json(requests).status(200);
            })
            .catch((error: Error) => {
   ...
   *          description: Internal server error
   */
  public static getAllUsersFromFriendRequests: RequestHandler = async (
    req,
    res
  ) => {
    const username: string = req.params.user;
    ...
    if (username == null) {
      ...
      await UserModel.findOne({ username: username }).then(async (user) => {
        if (user == null) {
          ...
          await FriendRequestModel.find({ from_user: username })
            .then(async (requests) => {
              return res.json(requests).status(200);
            })
            .catch((error: Error) => {
  ...
  public static acceptFriendRequest: RequestHandler = async (req, res) => {
    ...
    await FriendRequestModel.findOne({ request_id: request_id })
      .then(async (request) => {
        if (request == null) {
          return res.status(404).json({ error: "Request not found" });
        } else {
          await UserModel.findOne({ username: request.to_user }).then(
            async (to) => {
              if (to == null) {
                // This should never ever happen
                return res
                  .status(404)
                  .json({ error: "A user that should exist doesn't exist" });
              } else {
                await UserModel.findOne({ username: request.from_user }).then(
                  async (from) => {
                    if (from == null) {
                      // This should never ever happen
                      return res.status(404).json({
                        error: "A user that should exist doesn't exist",
                      });
                    } else {
                      if (to.friends == null) {
  ...
  public static search: RequestHandler = async (req, res) => {
    ...
    await UserModel.find({ username: { $regex: query, $options: "i" } })
      .then(async (users) => {
  ...
  public static sendCanvasRequest: RequestHandler = async (req, res) => {
    ...
    if (to === from)
      return res
        .status(403)
        .json({ error: "Cannot send canvas request to yourself" });

    if (!size) return res.status(400).json({ error: "canvas size is missing" });

    if (!roomID) return res.status(400).json({ error: "room ID is missing" });
          ...
          await UserModel.findOne({ username: from })
            .then(async (fromUser) => {
              if (fromUser == null) {
                ...
                await CanvasRequestModel.findOne({
                  to_user: to,
                  from_user: from,
                })
                  .then(async (request) => {
                    if (request == null) {
                      ...
                      await new CanvasRequestModel({ ...request })
                        .save()
                        .then(async () => {
                          logEvent(
                            fromUser,
  ...
  public static acceptCanvasRequest: RequestHandler = async (req, res) => {
    ...
    await CanvasRequestModel.findOne({ request_id: request_id })
      .then(async (request) => {
        if (request == null) {
          return res.status(404).json({ error: "Request not found" });
        } else {
   ...
   *          description: Internal server error
   */
  public static getAllUsersToCanvasRequests: RequestHandler = async (
    req,
    res
  ) => {
    const username: string = req.params.user;
    ...
    if (username == null) {
      ...
      await UserModel.findOne({ username: username }).then(async (user) => {
        if (user == null) {
          ...
          await CanvasRequestModel.find({ to_user: username })
            .then(async (requests) => {
              return res.json(requests).status(200);
            })
            .catch((error: Error) => {
            ...
            if (user_id) {
              await UserModel.findOne({ id: user_id }).then(async (user) => {
                if (!user) {
                  ...
                  await PostModel.find({ author: user })
                    .then(async (posts) => {
                      posts.map(async (post) => {
                        await post.remove().catch((e: Error) => {
                          return res.status(500).json({ error: e.name });
                        });
                      });
                    })
                  ...
                  await user
                    ...
                    .then(async () => {
                      return res
                        .status(200)
                        .json({ message: "Success removing user" });
                    })
                    .catch((e: Error) => {
                      return res.status(500).json({ error: e.name });
                    });
  ...
  public static getProfile: RequestHandler = async (req, res) => {
    ...
    await UserModel.findOne({ profileID: profileID }).then(async (user) => {
      if (!user) {
        ...
        await PostModel.find({ "author.id": user.id })
          .then(async (posts) => {
